...
class CustomClassifier(base.BaseEstimator, base.ClassifierMixin):
    ...
    def _fit_mult_est(self, X, y, **fit_params):
        def thread_fit(rfeX, rfe_key, est_key, results_dict):
            clc = clone(self.grid[est_key])
            results_dict[rfe_key][est_key] = clc.fit(rfeX, y)

        self.set_params(**fit_params)
        if isinstance(X, dict):
            ts=[]
            for rfe_sel, Xarr in X.items():
                for est_key in self.grid.keys():
                    ts.append(
                        threading.Thread(target=thread_fit, args=[Xarr, rfe_sel, est_key, self.output_models_])
                    )
